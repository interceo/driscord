...
Java_com_driscord_jni_NativeDriscord_screenInit(JNIEnv*, jclass, jint bufMs, jint maxSyncMs) {
...
JNIEXPORT void JNICALL
Java_com_driscord_jni_NativeDriscord_setOnFrame(JNIEnv* env, jclass, jobject cb) {
    if (!cb) {
...
JNIEXPORT void JNICALL
Java_com_driscord_jni_NativeDriscord_setOnFrameRemoved(JNIEnv* env, jclass, jobject cb) {
    CORE().set_on_frame_removed(make_string_cb(env, cb));
}

JNIEXPORT void JNICALL
Java_com_driscord_jni_NativeDriscord_screenJoinStream(JNIEnv* env, jclass, jstring jPeerId) {
    const char* peer = env->GetStringUTFChars(jPeerId, nullptr);
