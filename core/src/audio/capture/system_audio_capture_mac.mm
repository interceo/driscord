...
- (void)stream:(SCStream*)stream
    ...
    OSStatus
        status = CMBlockBufferGetDataPointer(blockBuffer, 0, nullptr, &totalLength, &dataPointer);
            ...
            getShareableContentWithCompletionHandler:
                ^(SCShareableContent* content, NSError* error) {
                  ...
                  SCContentFilter* filter = [[SCContentFilter
                      alloc] initWithDisplay:content.displays.firstObject
                  ...
                  capture->stream_          = [[SCStream
                      alloc] initWithFilter:filter configuration:config delegate:nil];
                  capture->output_          = [[DRAudioStreamOutput alloc] init];
                  capture->output_.callback = capture->callback_;

                  NSError* addErr = nil;
                  [capture->stream_
                         addStreamOutput:capture->output_
