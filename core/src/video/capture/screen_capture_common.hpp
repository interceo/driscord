...
inline void scale_nearest(const uint8_t* src, int sw, int sh, uint8_t* dst, int dw, int dh) {
    for (int y = 0; y < dh; ++y) {
        int sy = y * sh / dh;
        for (int x = 0; x < dw; ++x) {
            int sx            = x * sw / dw;
...
inline void compute_output_size(
    ...
    if (out_w > max_w || out_h > max_h) {
        float
            scale = std::min(static_cast<float>(max_w) / out_w, static_cast<float>(max_h) / out_h);
