    ...
    size_t payload_len)
{
    ...
    if (sender_id.size() > kMaxRelayedMediaSenderLen) {
    ...
    const size_t header_len = kRelayedMediaSenderLenSize + sender_id.size();
    ...
    dst[0] = static_cast<uint8_t>(sender_id.size());
    std::memcpy(dst + kRelayedMediaSenderLenSize, sender_id.data(),
        sender_id.size());
    std::memcpy(dst + kRelayedMediaSenderLenSize + sender_id.size(), payload,
        payload_len);
    return out;
}
