...
TEST_F(MediaDumpTest, WavRoundTripHasCanonicalPatchedHeader)
{
    ...
    EXPECT_EQ(u16_le(bytes, 20), 1u);
    EXPECT_EQ(u16_le(bytes, 22), 2u);
    EXPECT_EQ(u32_le(bytes, 24), 48'000u);
    EXPECT_EQ(u32_le(bytes, 28), 48'000u * 4u);
    EXPECT_EQ(u16_le(bytes, 32), 4u);
    EXPECT_EQ(u16_le(bytes, 34), 16u);
    EXPECT_EQ(u32_le(bytes, 40), 12u);
