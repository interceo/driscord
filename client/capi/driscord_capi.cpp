...
const char* driscord_local_id(void) {
    return dup_str(CORE().local_id());
}
const char* driscord_peers(void) {
    return dup_str(CORE().peers_json());
}

void driscord_set_on_peer_joined(DriscordStringCb cb) {
    CORE().set_on_peer_joined(wrap_str_cb(cb));
}
void driscord_set_on_peer_left(DriscordStringCb cb) {
    CORE().set_on_peer_left(wrap_str_cb(cb));
}
void driscord_set_on_streaming_started(DriscordStringCb cb) {
    CORE().set_on_streaming_started(wrap_str_cb(cb));
}
void driscord_set_on_streaming_stopped(DriscordStringCb cb) {
    CORE().set_on_streaming_stopped(wrap_str_cb(cb));
}
...
void driscord_audio_on_peer_joined(const char* peer, int jitter_ms) {
    CORE().audio_on_peer_joined(peer, jitter_ms);
}
...
void driscord_video_set_watching(bool watching) {
    CORE().video_set_watching(watching);
}
bool driscord_video_watching(void) {
    return CORE().video_watching();
}
...
void driscord_set_on_new_streaming_peer(DriscordStringCb cb) {
    CORE().set_on_new_streaming_peer(wrap_str_cb(cb));
}
void driscord_set_on_streaming_peer_removed(DriscordStringCb cb) {
    CORE().set_on_streaming_peer_removed(wrap_str_cb(cb));
}
...
bool driscord_capture_system_audio_available(void) {
    return CORE().capture_system_audio_available();
}
const char* driscord_capture_video_list_targets(void) {
    return dup_str(CORE().capture_video_list_targets_json());
}

int driscord_capture_grab_thumbnail(
...
void driscord_screen_init(int buf_ms, int max_sync_ms) {
    CORE().init_screen_session(buf_ms, max_sync_ms);
}
void driscord_screen_deinit(void) {
    CORE().deinit_screen_session();
}

bool driscord_screen_start_sharing(
    const char* target_json,
    int max_w,
    int max_h,
    int fps,
...
bool driscord_screen_sharing(void) {
    return CORE().screen_sharing();
}
bool driscord_screen_sharing_audio(void) {
    return CORE().screen_sharing_audio();
}
...
const char* driscord_screen_active_peer(void) {
    return dup_str(CORE().screen_active_peer());
}
bool driscord_screen_active(void) {
    return CORE().screen_active();
}
...
void driscord_screen_set_stream_volume(const char* peer, float vol) {
    CORE().screen_set_stream_volume(peer, vol);
}
float driscord_screen_stream_volume(void) {
    return CORE().screen_stream_volume();
}
const char* driscord_screen_stats(void) {
    return dup_str(CORE().screen_stats_json());
}
...
void driscord_set_on_frame_removed(DriscordStringCb cb) {
    CORE().set_on_frame_removed(wrap_str_cb(cb));
}
void driscord_screen_join_stream(const char* peer_id) {
    CORE().join_stream(peer_id);
}
