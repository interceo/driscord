...
class RingBuffer {
public:
    explicit RingBuffer(size_t capacity) : buf_(capacity + 1), capacity_(capacity + 1) {}

    size_t available_read() const {
        size_t w = write_pos_.load();
        size_t r = read_pos_.load();
    ...
    size_t write(const T* data, size_t count) {
        ...
        write_pos_.store((w + count) % capacity_);
        return count;
    }
    ...
    size_t read(T* data, size_t count) {
        ...
        read_pos_.store((r + count) % capacity_);
        return count;
    }
