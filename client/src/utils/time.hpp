...
inline WallTimestamp WallNow() { return std::chrono::system_clock::now(); }
...
inline WallTimestamp WallFromMs(uint64_t ms) { return WallTimestamp{std::chrono::milliseconds(ms)}; }
