...
bool AudioEngine::start(PacketCallback on_packet) {
    ...
    if (ma_device_init(nullptr, &config, device_.get()) != MA_SUCCESS) {
        ...
        opus_decoder_destroy(decoder_); decoder_ = nullptr;
        device_.reset();
        return false;
    }
...
void AudioEngine::on_capture(const float* input, uint32_t frames) {
    ...
    input_level_.store(rms, std::memory_order_relaxed);
    ...
    while (consumed < frames) {
        uint32_t to_copy = std::min(
            static_cast<uint32_t>(FRAME_SIZE - capture_pos_),
            frames - consumed);
        ...
        if (capture_pos_ == static_cast<size_t>(FRAME_SIZE)) {
            int bytes = opus_encode_float(
                encoder_, capture_buf_.data(), FRAME_SIZE,
                encode_buf_.data(), MAX_OPUS_PACKET);
...
void AudioEngine::on_playback(float* output, uint32_t frames) {
    ...
    float vol = output_volume_.load(std::memory_order_relaxed);
    float sum = 0.0f;
    for (uint32_t i = 0; i < frames; ++i) {
        output[i] *= vol;
        sum += output[i] * output[i];
    }
    output_level_.store(std::sqrt(sum / static_cast<float>(frames)), std::memory_order_relaxed);
}

void AudioEngine::feed_packet(const uint8_t* data, size_t len) {
    if (!running_ || !decoder_) return;

    int samples = opus_decode_float(
        decoder_, data, static_cast<int>(len),
        decode_buf_.data(), FRAME_SIZE, 0);
