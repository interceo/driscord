...
static constexpr auto LOG_CAPACITY = 2000;

App::App() {
    logs_.reserve(LOG_CAPACITY);
...
std::vector<App::PeerView> App::peers() const {
    ...
    result.reserve(transport_.peers().size());
...
void App::log(LogEntry::Level lvl, const std::string& text) {
    ...
    if (logs_.size() > LOG_CAPACITY) {
        logs_.erase(logs_.begin(), std::next(logs_.begin(), 500));
    }
}
