...
схемы; полноценный Alembic workflow пока отсутствует. Аватары хранятся в
`DATA_DIR` и требуют общего object storage при нескольких экземплярах API.

Обновления клиента через API не проходят: release-builder публикует подписанные
minisign манифесты (`latest.json` + `.minisig`) и immutable-архивы на анонимный
статический канал (`https://updates.homelab.ceooptimizator.tech/driscord/
<channel>/<target>/`), а клиент сам проверяет манифест вкомпилированным списком
доверенных ключей, сверяет sha256 архива и применяет обновление только по
действию пользователя. Клиентские бандлы собираются по тегам `client-vX.Y.Z`, образы
сервера — по `server-vX.Y.Z` (см. «Релизы» в `docs/deployment.md`).
