...
Сервер и клиент выпускаются раздельно, по префиксу тега:

- `server-vX.Y.Z` — release-builder собирает образы `driscord-api` и
  `driscord-signaling`, Flux выкатывает их в кластер;
- `client-vX.Y.Z` — release-builder собирает Linux/Windows/macOS-бандлы и
  публикует их в канал обновлений.

Суффикс `-rc.N` делает релиз prerelease: бандлы уходят в канал `beta`, Flux
такие образы не выкатывает. Релиз создаётся кнопкой: Actions → Release → Run
workflow, поля «компонент», «версия», «dry run». Workflow проверяет, что тега
ещё нет, что версия выше последнего релиза этого потока (и старых общих `v*`
тегов) и что CI на коммите зелёный, после чего создаёт тег и релиз. Версии
протокола пока нет: ломающее изменение протокола выпускается сначала клиентом,
потом сервером.
