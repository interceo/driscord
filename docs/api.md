...
Обновлений клиента в API нет: клиент проверяет и скачивает релизы напрямую со
статического канала обновлений (см. `docs/architecture.md`), проверяя
minisign-подпись манифеста вкомпилированным ключом.
