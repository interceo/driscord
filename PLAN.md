...
- [ ] Перевести видеопуть с `QQuickImageProvider` на per-peer `QVideoSink` +
  `VideoOutput`: сейчас каждый кадр копируется трижды (I420→RGBA на потоке
  декодера, `QImage::copy` в `FrameProvider`, перезагрузка `Image` через
  `source=""`), плюс `I420Buffer::Create` и вектор масштабирования на каждый
  кадр в `submit_bgra`. **Заблокировано** отсутствием сборки клиента на рабочей
  машине: правка лежит на горячем пути кадров и без компиляции и живого прогона
  не отдаётся. Разблокируется этапом E2 из `PLAN3.md`.
- [ ] Собирать и выпускать релизные бинари под Linux и Windows и запускать
  автосборку на push в `main` — отдельный план: `PLAN3.md`.
...
- pinned Google WebRTC artifact сейчас поддержан только на Linux x86_64 и
  ...
  (план получения Windows-артефакта — `PLAN3.md`);
- релизный Linux-бинарь пока непереносим: сборка идёт с `use_sysroot=false` и
  линкуется против glibc сборочной машины (на текущем хосте 2.43), то есть не
  запустится на Ubuntu 24.04 или Debian 12. Лечится сборкой всего конвейера
  внутри контейнера с нижней целевой glibc, см. `PLAN3.md` E2;
